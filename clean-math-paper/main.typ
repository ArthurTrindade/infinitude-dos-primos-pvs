...
$("topology?")$ Uma topologia sobre um conjunto $X$ é uma coleção $tau$ subconjuntos de $X$
que satisfaz as seguintes propriedades:

1. $emptyset$ e $X$ pertencem a $tau$ $("topology_empty? e topology_full?")$;
2. A união dos elementos de qualquer subcoleção de $tau$ pertence a $tau$ $("union_open_is_open e any_union_open_is_open")$;
3. A interseção dos elementos de uma subcoleção finita de $tau$ pertence a $tau$ $("inter_open_is_open")$.
...
#proof[
  Seja $ A subset.eq tau$ e $ Y = union.big_(O_a in A) O_alpha $
...
#proof[
  Seja $A subset.eq tau$, com $A$ finito e $|A| = n$. Podemos provar por indução na quantidade de conjuntos na interseção.
  ...
  $ inter.big_(i = 1)^n O_i = (inter.big_(i = 1)^(n-1) O_i) inter O_n $

  Pela _H.I._ $inter.big_(i = 1)^(n-1) O_i in tau$

  - Se $(inter.big_(i = 1)^(n-1) O_i) inter O_n = emptyset in tau$

  - Se $(inter.big_(i = 1)^(n-1) O_i) inter O_n != emptyset$, então $exists a in (inter.big_(i = 1)^(n - 1) O_i) inter O_n$. Logo, $exists b_i, b_n$, tais que $N_(a, b_i) subset.eq (inter.big_(i = 1)^(n - 1) O_i)$ e $N_(a, b_n) subset.eq O_n$. Portanto, $N_(a, b_i dot b_n) subset.eq (inter.big_(i = 1)^(n-1) O_i) inter O_n$.
]
...
#definition(title: "Conjuntos Fechados")[
  Um subconjunto $A$ de um espaço topológico $X$ é chamado conjunto fechado se, e somente se, seu complementar $A^c$ é um conjunto aberto em $X$.
]

#proposition[
  Para quaisquer $a in bb(Z) "e" b > 0, Nab "é fechado"$.

  $ Nab = bb(Z) - union.big_(i = 1)^(b - 1) N_(a + i, b) \ "e" \
    union.big_(i = 1)^(b - 1) N_(a + i, b) "é um conjunto aberto"
  $ 
]

#lemma[
  *Lema de Infinitude* $"(open_is_infinite)"$: Todo aberto não vazio contém uma progressão $N_(a,b)$, sendo, portanto, infinito.
]

#lemma[
  *Lema do Conjunto Fechado* $"(fin_prime_closed_Np)"$: Cada $N_(a,b)$ é simultane amente aberto e fechado, pois seu complementar é uma união de outros abertos $N_(a+i,b)$.
]
...
#proof[
  ...
  - Consequentemente, ${-1, 1}$ é aberto infinito. Absurdo.
...
- *skeep* (Skolemization): Para eliminar quantificadores universais e introduzir constantes arbitrárias.
- *split*: Para ramificar a prova em casos menores (ex: tratar separadamente o caso do conjunto vazio).
- *flatten*: Para simplificar a estrutura lógica das fórmulas no sequente.
...
união de todos os $N_(0,p)$ para $p in PP$ (primos) cobre $ZZ without {-1, 1}$. Se os primos fossem finitos, essa união seria um conjunto fechado, tornando seu complementar ${-1, 1}$ um conjunto
aberto. Como ${-1, 1}$ é finito e não vazio, ele contradiz o lema $"open_is_infinite"$,
provando que o conjunto dos primos é necessariamente infinito.
